...
#define SERVER_IP                                                              \
  "127.0.0.1" // 서버의 IP 주소 (여기서는 자기 자신, 즉 localhost)
...
int main(int argc, char *argv[]) {
  ...
  sprintf(message, "%s:Just Connected", my_id);
  ...
  if (len > 0) {
    auth_buffer[len] = 0;
    if (strcmp(auth_buffer, "DENIED") == 0) { // 서버에서 확인되지 않는 sensor
                                              // ID
      printf("\n[오류] 존재하지 않는 센서 ID입니다: %s\n", my_id);
      printf("올바른 ID로 다시 실행해주세요.\n");
      close(sock);
      return 0;
    } else if (strcmp(auth_buffer, "ACCEPTED") ==
               0) { // 서버에서 확인된 sensor ID
  ...
  while (1) {
    // 사용자 입력 대기
    printf("상태 입력 (예: OK, ERROR): "); // 센서에 특정 조건이 걸리면 에러를
                                           // 출력하도록 하기 위함
