...
void play_intro() {
  ...
  for (int i = 0; i < 150; i++) {
    ...
    init_pair(4, font_color, COLOR_BLACK);
    attron(COLOR_PAIR(4));
    fseek(f, SEEK_SET, 0);

    for (int j = 0; j < 40; j++) {
      fread(buffer, sizeof(char), 164, f);
      buffer[164] = '\0';
      mvprintw(j + 4, 8, "%s", buffer);
    }
  ...
  while (1) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
      ...
      if (active_clients[i]) {
        ...
        mvprintw(row, 2, "[Machine %s] Waiting for connection...",
                 SENSOR_IDS[i]); // 프로토콜 구체화 필요
...
void *handle_client(void *arg) {
  ...
  while (1) {
    ...
    if (id == -1) { // ui 상에서 아직 자리가 배정 안 됐다면
      char temp_id[20];
      sscanf(buffer, "%[^:]",
             temp_id); // 메시지에서 ':' 앞부분까지만 복사해서 temp_id에 저장
